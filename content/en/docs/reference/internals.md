...
* If ```JSON```: Parse JSON body and set ARGS variables
...
The ``tx.ProcessRequest(req)`` method is a convenience wrapper that takes a standard ``*http.Request`` and calls the individual processing methods (``ProcessConnection``, ``ProcessURI``, ``ProcessRequestHeaders``, and ``ProcessRequestBody``) in the correct order. This simplifies integration when you have access to the full ``http.Request`` object.

For Go HTTP servers, the ``github.com/corazawaf/coraza/v3/http`` package provides ``txhttp.WrapHandler``, which wraps any ``http.Handler`` with Coraza WAF protection. It handles the full request/response lifecycle including connection processing, header inspection, body buffering, and response interception.

## HTTP Server Example

The [http-server example](https://github.com/corazawaf/coraza/tree/main/examples/http-server) in the Coraza repository demonstrates how to integrate Coraza with a standard Go HTTP server:
...
This is the recommended starting point for integrating Coraza into Go applications. The ``txhttp.WrapHandler`` approach handles all the phase processing automatically, so you don't need to call ``ProcessConnection``, ``ProcessURI``, etc. manually.
