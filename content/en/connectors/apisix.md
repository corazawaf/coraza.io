...
Mind that a WAF is not a plug-and-play security solution. It requires a configuration and tuning tailored to the environment and traffic the WAF is meant to protect to be effective. For production usage, it is strongly recommended to be fully aware of the deployed configurations (See [@recommended-conf](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/wasmplugin/rules/coraza.conf-recommended.conf) and [@crs-setup-conf](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/wasmplugin/rules/crs-setup.conf.example)) and to perform a tuning phase of the rule set used. For more information on tuning the OWASP Core Rule Set (CRS), please refer to the [False Positives and Tuning](https://coreruleset.org/docs/concepts/false_positives_tuning/) guide.
...
Up to date required versions can be found looking at [`minGoVersion` and `tinygoMinorVersion` variables](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/magefiles/magefile.go).
...
By default, coraza-proxy-wasm runs with multiphase evaluation enabled (See [coraza.rule.multiphase_evaluation](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/.magefiles/magefile.go) build tag). It enables the evaluation of rule variables in the phases that they are ready for, potentially anticipating the phase the rule is defined for. This feature suits coraza-proxy-wasm, and specifically Envoy request lifecycle, aiming to inspect data that has been received so far as soon as possible. It leads to enforce actions the earliest possible, avoiding WAF bypasses. This functionality, in conjunction with the [early blocking CRS feature](#recommendations-using-crs-with-proxy-wasm), permits to effectively raise the anomaly score and eventually drop the request at the earliest possible phase.
...
- In order to mitigate as much as possible malicious requests (or connections open) sent upstream, it is recommended to keep the [CRS Early Blocking](https://coreruleset.org/20220302/the-case-for-early-blocking/) feature enabled (SecAction [`900120`](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/wasmplugin/rules/crs-setup.conf.example)).
...
Take a look at its config file [ftw.yml](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/ftw/ftw.yml) for details about tests currently excluded.
...
For details and locally tweaking the configuration refer to [@recommended-conf](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/wasmplugin/rules/coraza.conf-recommended.conf) and [@crs-setup-conf](https://github.com/corazawaf/coraza-proxy-wasm/blob/main/wasmplugin/rules/crs-setup.conf.example).
